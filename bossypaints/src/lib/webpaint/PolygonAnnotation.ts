...
class PolygonAnnotation {
	...
	draw(p: p5, nav: NavigationStore, annoMgr: AnnotationManagerStore) {
		...
		const displayColor = annoMgr.getSegmentColor(this.segmentID);

		// Pick a fill color:
		const opacity =
			255 *
			(this.editing
				? APP_CONFIG.editingOpacity
				: hover
					? APP_CONFIG.hoveredOpacity
					: annoMgr.currentSegmentID === this.segmentID
						? APP_CONFIG.activeOpacity
						: APP_CONFIG.nonActiveOpacity);
		p.fill(displayColor[0], displayColor[1], displayColor[2], opacity);
		p.stroke(displayColor[0], displayColor[1], displayColor[2], opacity + 10);
		p.strokeWeight(2);
		...
		if (
			hover &&
			!this.editing &&
			this.positiveRegions.length > 0 &&
			this.positiveRegions[0].length > 0
		) {
			const canonicalSegmentID = annoMgr.getCanonicalSegmentID(this.segmentID);
			const hoverLabel =
				canonicalSegmentID === this.segmentID
					? `Segment: ${this.segmentID}`
					: `Segment: ${canonicalSegmentID} (${this.segmentID})`;
			p.fill(0, 0, 0, 255);
			p.textSize(12 / nav.zoom);
			p.noStroke();
			p.text(hoverLabel, this.positiveRegions[0][0][0], this.positiveRegions[0][0][1]);
		}
	}
	...
	private isRegionContainedInRegion(
		holeRegion: Array<[number, number]>,
		containerRegion: Array<[number, number]>
	): boolean {
