...
export class ImageCache {
	...
	constructor(
		...
		if (
			bossRemoteOrOptions &&
			typeof bossRemoteOrOptions === 'object' &&
			!('protocol' in bossRemoteOrOptions)
		) {
	...
	set(identifier: ChunkIdentifier, image: any): void {
		...
		debugUtil.log(
			`CACHE SET: ${key} (resolution ${identifier.resolution}) - Cache size: ${this.cache.size} items, ${(this.currentSize / 1024 / 1024).toFixed(1)}MB`
		);
	...
	async getImage(identifier: ChunkIdentifier): Promise<any> {
		...
		if (!this.bossRemote || !this.datasetURI) {
			debugUtil.warn(
				'ImageCache.getImage: Image not in cache and BossRemote/URI not available for loading'
			);
			return null;
		}
		...
		requestPromise = (async () => {
			try {
				...
				const image = await this.blobToP5Image(blob);
				if (!image) {
					return null;
				}
				...
				const current = this.activeRequests.get(requestKey);
				if (requestPromise && current?.promise === requestPromise) {
					this.activeRequests.delete(requestKey);
				}
			}
		})();

		this.activeRequests.set(requestKey, { controller, promise: requestPromise });
		return requestPromise;
	}

	/**
	 * Abort any in-flight requests that are no longer part of the desired visible set.
	 */
	cancelRequestsExcept(identifiers: ChunkIdentifier[]): void {
		const allowedKeys = new Set(identifiers.map((identifier) => this.generateKey(identifier)));
	...
	private async blobToP5Image(blob: Blob): Promise<any | null> {
		if (!this.p5Instance) {
			return null;
		}
		...
		try {
			...
			debugUtil.warn('ImageCache: Failed to decode image blob', error);
			return null;
		}
	}
	...
	private evictOldWindows(
		perChunk: Map<string, { image: any; timestamp: number }>,
		maxPerChunk: number = 4
	) {
	...
	async preloadNeighboringChunks(centerIdentifier: ChunkIdentifier, radius: number): Promise<void> {
		...
		debugUtil.log(
			`ImageCache: Would preload ${radius} radius neighbors around chunk`,
			centerIdentifier
		);
	}
	...
	evictResolutionLevel(resolutionLevel: number): void {
		...
		debugUtil.log(
			`ImageCache: Evicted ${toRemove.length} chunks from resolution level ${resolutionLevel}`
		);
	}
	...
	getFilmstripRenderInfo(identifier: ChunkIdentifier, targetLayer?: number): any | null {
		...
		if (layerToExtract < identifier.z_min || layerToExtract >= identifier.z_max) {
			debugUtil.warn(
				`Target layer ${layerToExtract} is outside filmstrip range [${identifier.z_min}, ${identifier.z_max})`
			);
			return null;
		}
		...
		debugUtil.log(
			`Filmstrip extraction: layer ${layerToExtract}, offset ${layerOffsetInFilmstrip}, sourceY=${sourceY}, sourceHeight=${sourceHeight}, total height=${filmstripChunk.image.height}`
		);
