<script lang="ts">
	...
	import {
		generateNeuroglancerLink,
		generateNeuroglancerLinkWithSegmentation
	...
	const totalExports = $derived(
		(exports?.meshes?.length || 0) +
			(exports?.segments?.length || 0) +
			(exports?.cloudvolumes?.length || 0)
	);
		...
		<div class="bg-white rounded-2xl shadow-sm p-8 border border-gray-200 mb-8">
			<div class="flex items-center justify-between mb-6">
				...
				<div class="flex space-x-3 flex-wrap space-y-3">
					...
					<a
						href={generateNeuroglancerLink(task)}
						target="_blank"
					...
					{#if exports?.cloudvolumes && exports.cloudvolumes.length > 0}
						<div class="bg-gray-50 rounded-xl p-6">
							<div class="flex items-center mb-4">
								...
								<h3 class="text-lg font-medium text-gray-900">
									CloudVolume Exports ({exports?.cloudvolumes?.length || 0})
								</h3>
							</div>
							<div class="space-y-2 max-h-60 overflow-y-auto">
								{#each exports.cloudvolumes as cv}
									<div
										class="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200 hover:border-yellow-200 transition-colors duration-200"
									>
										<div class="flex items-center min-w-0 flex-1">
											<div class="min-w-0 flex-1">
												<p class="text-sm font-medium text-gray-900 truncate">{cv.filename}</p>
												<p class="text-xs text-gray-500">
													{formatFileSize(cv.size)} • {formatDate(cv.modified)}
												</p>
											</div>
										</div>
										<div class="ml-3 flex items-center space-x-2">
											<a
												href={getDownloadUrl(cv.filename)}
												download
												class="inline-flex items-center px-2 py-1 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 text-xs font-medium rounded transition-colors duration-200"
											>
												<svg
													class="w-3 h-3 mr-1"
													fill="none"
											...
											{#if cv.uri}
												<a
													href={generateNeuroglancerLinkWithSegmentation(task, cv.uri)}
													target="_blank"
													class="inline-flex items-center px-2 py-1 bg-gray-200 hover:bg-gray-300 text-gray-800 text-xs font-medium rounded transition-colors duration-200"
												>
													<svg
														class="w-3 h-3 mr-1"
														fill="none"
